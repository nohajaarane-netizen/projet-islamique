...
export default function AsmaUlHusna() {
  ...
  const filtered = allNames.filter(n =>
    n.translit.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div style={{ background: C.cardBg, borderRadius: 24, padding: 24, border: `1px solid ${C.border}` }}>
        <h2 style={{ marginBottom: 8, color: C.textDark }}>{t('asma.title')}</h2>
        <p style={{ marginBottom: 20, color: C.textMid }}>{t('asma.subtitle')}</p>

        <div style={{ marginBottom: 24, textAlign: 'center' }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: C.green }}>
            {learned.length} / {allNames.length}
          </div>
        ...
        <input
          type="text"
          placeholder={t('asma.search_placeholder')}
          value={search}
          onChange={e => setSearch(e.target.value)}
          style={{ width: '100%', padding: 12, borderRadius: 40, marginBottom: 24, border: `1px solid ${C.border}`, background: C.cardBg2, color: C.textDark }}
        />
          ...
          {filtered.map(n => (
            <div
              ...
              style={{
                display: 'flex',
                alignItems: 'center',
                background: learned.includes(n.id) ? C.cardBg2 : C.cardBg,
            ...
            >
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 20, fontFamily: "'Amiri', serif", color: C.textDark }}>{n.name}</div>
                <div style={{ fontWeight: 600, color: C.textDark }}>{n.translit}</div>
                <div style={{ fontSize: 12, color: C.textMid }}>{n.meaning}</div>
              </div>
              <div style={{ fontSize: 20, color: C.green }}>{learned.includes(n.id) ? '✓' : '○'}</div>
            </div>
          ))}
        </div>
      </div>
